#modified by Martin Rittner, 2013, to allow overriding automatic bandwidth calculation
#removed some Matlab idiosyncrasies uncommon to R, in particular:
#removed semicolons at end of lines
#resolved multiple expressions on the same line
#replaced Matlab-style "nargin" parameter check by "missing()"
#added explicit "return()" at end of functions



pkde <- function(data,bandwidth,n,MIN,MAX){
...
# 	if (nargin<2) n=2^14
	if(missing(n))n<-2^14	#replacement of unreliable "nargin"
...
# 	}
	...
	t_star<-ifelse(missing(bandwidth),
								 tryCatch(uniroot(fixed_point,c(0,.1),N=N,I=I,a2=a2,tol=10^(-14))$root,
								 				 error=function(e){return(.28*N^(-2/5))}),
								 (bandwidth/R)^2
	)
...
#################################################
#dirty hack to read the optimal bandwidth out:
optimal_bw<- function(data,n,MIN,MAX){
	if(missing(n))n<-2^12
	n<-2^ceiling(log2(n)); # round up n to the next power of 2;
	minimum<-min(data)
	maximum<-max(data)
