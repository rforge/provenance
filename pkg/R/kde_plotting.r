...
plotKDE<-function(data,title,limits=c(0,3000),breaks=NA,bandwidth=NA,
	...
	# TODO: take data.frame, list, or geoDataSet as input, format accordingly in here (check geoDataSet for errors, use for spot indicators?)
	...
	if(is.data.frame(data)){
		if(all(sapply(data[plotonly],is.numeric))){
			data<-data[,plotonly]
		}else{
	...
	if(logx){
		...
		if(any(limits<=0))limits[limits<=0]<-min(sapply(data,min,na.rm=TRUE))
